...
if(isset($_SESSION['username'])){
	...
	$sql2= "insert into khach_hang(cmnd,tenkh,email,sdt,dia_chi) values('".$cmnd."','".$tenkh."','".$email."','".$sdt."','".$diachi."')";
	$exec1= mysqli_query($connect,$sql1);
	$last_id_hd = mysqli_insert_id($connect);
	$exec2= mysqli_query($connect,$sql2);
	$last_id_kh = mysqli_insert_id($connect);
	
		if($exec1){

	$sql= "update san_pham set mua_nhieu=mua_nhieu + 1 where masp in (".$list_id.")";
	$sql3 = "insert into don_hang(ma_hoadon,ma_kh,tenkh,tongtien) values('".$last_id_hd."','".$last_id_kh."','".$tenkh."',".$tong_gia.")";
